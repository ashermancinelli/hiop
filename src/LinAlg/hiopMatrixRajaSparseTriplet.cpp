...
hiopMatrixRajaSparseTriplet::hiopMatrixRajaSparseTriplet(int rows, int cols, int nnz_)
...
{
  return new hiopMatrixRajaSparseTriplet(nrows, ncols, nnz);
}
...
hiopMatrix* hiopMatrixRajaSparseTriplet::new_copy() const
{
  ...
  assert(this->checkIndexesAreOrdered());
#endif
  hiopMatrixRajaSparseTriplet* copy = new hiopMatrixRajaSparseTriplet(nrows, ncols, nnz);
  memcpy(copy->iRow, iRow, nnz*sizeof(int));
  memcpy(copy->jCol, jCol, nnz*sizeof(int));
  memcpy(copy->values, values, nnz*sizeof(double));
  return copy;
}
void hiopMatrixRajaSparseTriplet::copyFrom(const hiopMatrixRajaSparseTriplet& dm)
{
...
addMDinvNtransToSymDeMatUTri(int row_dest_start, int col_dest_start,
			     const double& alpha, 
			     const hiopVectorPar& D, const hiopMatrixRajaSparseTriplet& M2,
...
{
  assert(nrows == ncols);
  return new hiopMatrixRajaSymSparseTriplet(nrows, nnz);
}
hiopMatrix* hiopMatrixRajaSymSparseTriplet::new_copy() const
{
  assert(nrows == ncols);
  hiopMatrixRajaSymSparseTriplet* copy = new hiopMatrixRajaSymSparseTriplet(nrows, nnz);
  memcpy(copy->iRow, iRow, nnz*sizeof(int));
  memcpy(copy->jCol, jCol, nnz*sizeof(int));
  memcpy(copy->values, values, nnz*sizeof(double));
  return copy;
